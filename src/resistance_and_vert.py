...
def findAlphaP(alphaP, y, vert, ode_parameters):
    '''
...
if __name__ == '__main__':
    ...
    VERT_MODERATE = 0.5
    VERT_HIGH = 0.8
    vert_vector = np.array([VERT_LOW, VERT_MODERATE, VERT_HIGH])
    nVert = len(vert_vector)

    # Storage matrix
    # Will be a nRes x nc1 x 6 matrix. 
    # Each axis 0 corresponds to a resistance value.
    # Axis 1 are cost values
    # Axis 2 is evolved virulence branches (2), H, D, P and B as functions of resistance and cost
    storageMat = np.zeros((nRes, nVert, 6))
    ...
    # Save directory
    saveDir = './data/paper_data/supp_fig/'
    ...
    if runData:
        ...
        for yInd, y in tqdm(enumerate(resistance_vector), total=len(resistance_vector)):
            ...
            for vertInd, vert in enumerate(vert_vector):

                # Update the ODE parameters
                ode_parameters['rho'] = vert

                optAlphaP = fsolve(lambda alphaP: findAlphaP(alphaP, y, vert, ode_parameters), x0=ancestral_virulence)
                storageMat[yInd, vertInd, 0] = optAlphaP[0]
                storageMat[yInd, vertInd, 1] = optAlphaP[0]

                # Now calculate the ecological dynamics at this value
                ode_parameters['alpha_P'] = storageMat[yInd, vertInd, 0]
                Ht, Dt, Pt, Bt = run_ode_solver(ode_parameters,
                            make_system(ode_parameters))
                
                # Store
                storageMat[yInd, vertInd, 2:] = [Ht[-1], Dt[-1], Pt[-1], Bt[-1]]
    ...
    # Create the figures
    for vertInd, vert in enumerate(pdict['vert_vector']):

        # Create a mask where the parasite dies out
        mask = np.sum(pdict['storageMat'][:, vertInd, 4:], axis=1) > extinction_threshold
        finalInd = sum(mask) - 1

        # Figure for relative virulence
        ax1.plot(pdict['resistance_vector'][mask], pdict['storageMat'][mask, vertInd, 0]/pdict['ancestral_virulence'], lw=4, c=cols[vertInd], ls=styles[vertInd], label = rf'$\rho$ = {vert}')
        ax1.plot(pdict['resistance_vector'][mask], pdict['storageMat'][mask, vertInd, 1]/pdict['ancestral_virulence'], lw=4, c=cols[vertInd], ls=styles[vertInd])
        if finalInd+1 != len(pdict['resistance_vector']):
            ax1.plot(pdict['resistance_vector'][finalInd], pdict['storageMat'][finalInd, vertInd, 0]/pdict['ancestral_virulence'], f'{cols[vertInd]}.', markersize=20)
            ax1.plot(pdict['resistance_vector'][finalInd], pdict['storageMat'][finalInd, vertInd, 1]/pdict['ancestral_virulence'], f'{cols[vertInd]}.', markersize=20)

        # Figure for change in host population size
        ax2.plot(pdict['resistance_vector'][mask], np.sum(pdict['storageMat'][mask, vertInd, 2:], axis=1)/np.sum(pdict['ancestral_steady_state']), lw=4, c=cols[vertInd], ls=styles[vertInd])
        if finalInd+1 != len(pdict['resistance_vector']):
            ax2.plot(pdict['resistance_vector'][finalInd], np.sum(pdict['storageMat'][finalInd, vertInd, 2:])/np.sum(pdict['ancestral_steady_state']), f'{cols[vertInd]}.', markersize=20)

        # Figure for change in parasitised hosts
        ax3.plot(pdict['resistance_vector'][mask], np.sum(pdict['storageMat'][mask, vertInd, 4:], axis=1)/pdict['ancestral_steady_state'][1], lw=4, c=cols[vertInd], ls=styles[vertInd])
        if finalInd+1 != len(pdict['resistance_vector']):
            ax3.plot(pdict['resistance_vector'][finalInd], np.sum(pdict['storageMat'][finalInd, vertInd, 4:])/pdict['ancestral_steady_state'][1], f'{cols[vertInd]}.', markersize=20)
